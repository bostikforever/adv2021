...
class Evaluator : PacketVisitor
{
    int packetCount;
    int versionCount;
    int openPackets;
    int versionSum;
    long expressionValue;

    int[] opStack, argCount;
    long[] valueStack;
    ...
    void recordType(int type)
    {
        opStack.assumeSafeAppend() ~= type;
    }

    void recordData(long data)
    {
        valueStack.assumeSafeAppend() ~= data;
    }
...
private:
    void apply(long value)
    {
        ...
        if (argCount[$ - 1] == 1)
        {
            valueStack.assumeSafeAppend() ~= value;
            return;
        }
        final switch (opStack[$ - 1])
        {
        case 0:
            valueStack[$ - 1] += value;
            break;
        ...
        case 2:
            ...
            valueStack[$ - 1] = min(valueStack[$ - 1], value);
            break;
        case 3:
            import std.algorithm : max;

            valueStack[$ - 1] = max(valueStack[$ - 1], value);
            break;
        case 5:
            assert(argCount[$ - 1] == 2);
            valueStack[$ - 1] = cast(int)(valueStack[$ - 1] > value);
            break;
        case 6:
            assert(argCount[$ - 1] == 2);
            valueStack[$ - 1] = cast(int)(valueStack[$ - 1] < value);
...
auto solve(string hexInputStream)
{
    ...
    hexInputStream.visit(evaluator);
    return evaluator.expressionValue;
}
