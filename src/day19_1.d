...
{
    import std.algorithm : cartesianProduct, count, each, filter, map, maxElement, sort, sum, uniq;
    ...
    writeln("Finished getting mapping transforms: ", transforms);
    ...
    writeln("Finished getting mapping paths: ", paths);
    int[][] sensorLocations;
    sensorLocations.length = sensorsData.length;
    sensorLocations[0] = [0, 0, 0];
    foreach (i; 1 .. sensorsData.length)
    {
        auto transformed = [0, 0, 0];
        ...
        foreach (currNext; slide(paths[i], 2))
        {
            ...
            foreach (tform; transforms[curr])
            {
                ...
                {
                    auto t = tform[1];
                    transformed = transformed.transform(t.index, t.axesSign)
                        .add(t.displacement);
                    break;
                }
        ...
        sensorLocations[i] = transformed;
    }
    return sensorLocations.cartesianProduct(sensorLocations).filter!"a[0] !is a[1]"
        .map!(
            a => a.expand.elementWise!((a, b) => abs(b - a)).sum)
        .maxElement;
}
